...
learn_rate = 0.014
train_steps = 100
train_output_folder = 'train-aac/'

bla = 0

def aac_calc_similarity(v, shift):
	if shift == 0:
		return 0.0
	...
	return ((v1-v2)**2).sum() / len(v2) * (shift + 256)
...
def aac_find_base(v):
	...
	for shift in range(40, 2*len(v) // 3):
		val = aac_calc_similarity(v, shift)
...
def my_convolve(a):
	...
	for i in range(len(a)):
		...
		hid = aac_find_base(a[i])

		v = a[i][:hid]
		v = resample(v, input_n)

		c[i] = np.array(np.abs(np.fft.fft(v * bh_window)),
			dtype=np.float32)[:input_processed_n]
